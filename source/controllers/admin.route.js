...
router.get('/account/instructor', async(req, res) => { // Tạo tài khoản cho intructor
    if (req.session.type !== "administrator")
        throw Error("Only admin can use this API");
    res.render('vwAdmin/addIntructor')
})

router.post('/account/instructor', async(req, res) => { // Tạo tài khoản cho intructor
    if (req.session.type !== "administrator")
        throw Error("Only admin can use this API");
    ...
    if (missing) {
        ...
        if (!validUsername || !validEmail) {
            return res.render('vwAdmin/addIntructor', {
                fail: "Please input a valid username and email",
            });
        }

        const hash = bcrypt.hashSync(req.body.password, 10);
        const accountIntructor = {
        ...
        var rows = await admin.createIntructor(accountIntructor);
        if (rows.error) {
            return res.render('vwAdmin/addIntructor', {
                fail: rows.error,
            });
